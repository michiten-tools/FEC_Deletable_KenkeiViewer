...
function dropKmlPin(latitude, longitude, when, direction, isGoogleType, movieFile) {

    // 方位に応じたKmlIcon取得して、KMLマーカー追加
    //var kmlIcon = iconKmlList[parseInt(direction)];
    var kmlIcon = getKmlIcon(isGoogleType, direction);
    var marker = L.marker([latitude, longitude], { icon: kmlIcon }).addTo(kmlGroup);

    // クリック時にC#側への通知（ファイル名も一緒に通知）
    marker.on('click', function (e) {
        chrome.webview.hostObjects.class.OnKmlClick(when, movieFile);
    });
}
...
function setIconSize(e) {
  ...
  var x = zoomLevel * 16 / 18;
    var y = zoomLevel * 16 / 18;
